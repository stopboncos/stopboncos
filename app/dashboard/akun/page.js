...
export default function AkunPage() {
  ...
  const handleSave = async () => {
    ...
    if (error) {
      ...
      await logActivity(user.id, 'akun', inserted.id, 'create', null, {
        name: form.name, type: form.type, balance: parseFloat(form.balance) || 0, color: form.color, notes: form.notes,
      })
      setShowModal(false)
  ...
  const handleDelete = async (akun) => {
    ...
    if (count > 0) {
      alert(`Tidak bisa dihapus — akun ini masih digunakan di ${count} transaksi.`)
      return
    }
    if (!confirm('Hapus akun ini?')) return
    const { data: { user } } = await supabase.auth.getUser()
    await logActivity(user.id, 'akun', akun.id, 'delete', {
      name: akun.name, type: akun.type, balance: akun.balance, color: akun.color, notes: akun.notes,
    }, null)
  ...
  return (
    <div style={{ width: '100%', boxSizing: 'border-box' }}>
      ...
      {loading ? (
        ...
        <div className="akun-grid">
          {akuns.map(akun => (
            <div
              key={akun.id}
              onClick={() => !isMobile && router.push(detailId === akun.id ? '?' : `?detail=${akun.id}`)}
              style={{
                background: 'var(--bg-card)',
                // borderTop: `4px solid ${akun.color}`,
                // borderRight: detailId === akun.id ? '1.5px solid var(--primary)' : '1px solid var(--border)',
                // borderBottom: detailId === akun.id ? '1.5px solid var(--primary)' : '1px solid var(--border)',
                // borderLeft: detailId === akun.id ? '1.5px solid var(--primary)' : '1px solid var(--border)',
                border: '1px solid var(--border) !important',
                borderTop: detailId === akun.id ? '4px solid var(--primary)' : '4px solid var(--border)',
            ...
            >
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '16px', gap: '8px' }}>
                ...
                <button
                  ...
                  onClick={(e) => { e.stopPropagation(); handleDelete(akun) }}
                  style={{ background: 'none', color: 'var(--danger)', border: 'none', padding: '4px', cursor: 'pointer', flexShrink: 0 }}
                >
              ...
              <div style={{ fontSize: '22px', fontWeight: '700', color: 'var(--text)' }}>{fmt(akun.balance)}</div>
              <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '4px' }}>Saldo saat ini</div>
              {akun.notes && (
                <div style={{ marginTop: '12px', fontSize: '12px', color: 'var(--text-muted)', background: 'var(--bg)', borderRadius: '6px', padding: '6px 10px', wordBreak: 'break-word' }}>{akun.notes}</div>
              )}
            </div>
          ))}
        </div>
      ...
      {/* Modal tambah */}
      {showModal && (
        <div onClick={() => setShowModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: '16px' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg-card)', borderRadius: '12px', width: '440px', maxWidth: '100%', padding: '24px', boxShadow: '0 8px 32px rgba(0,0,0,0.2)', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ marginBottom: '14px' }}>
              <label style={{ fontSize: '13px', fontWeight: '500', color: 'var(--text)', display: 'block', marginBottom: '6px' }}>Nama *</label>
              <input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="mis. BCA Utama, GoPay, Kas" style={inputStyle} />
            </div>
            <div style={{ marginBottom: '14px' }}>
              <label style={{ fontSize: '13px', fontWeight: '500', color: 'var(--text)', display: 'block', marginBottom: '6px' }}>Tipe Dompet</label>
            ...
            <div style={{ marginBottom: '14px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                {WARNA.map(w => (
                  <div key={w} onClick={() => setForm({ ...form, color: w })} style={{ width: '28px', height: '28px', borderRadius: '50%', background: w, cursor: 'pointer', border: form.color === w ? '3px solid var(--text)' : '3px solid transparent' }} />
                ))}
              </div>
            </div>
